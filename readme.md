...
格式選單也可將音訊轉換為 192 kbps MP3；所需的 FFmpeg 執行檔由 Python 相依套件提供。

首頁可貼上單部 YouTube 影片網址，影片會以 MP4 格式直接下載到本機 `media` 資料夾。此模式不需要 FFmpeg，但可用畫質取決於 YouTube 提供的單檔 MP4 格式。請只下載你有權保存的內容。
